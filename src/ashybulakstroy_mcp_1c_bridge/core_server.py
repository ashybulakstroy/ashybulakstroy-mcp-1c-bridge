...
def ask_1c(text: str, limit: int | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        if any(phrase in ql for phrase in ["кому мы должны", "кому должны", "кредиторка поставщикам", "задолженность поставщикам", "кому не заплатили поставщикам"]):
            date_from, date_to = _extract_date_range(q)
            counterparty = _extract_counterparty_hint(q)
            result = odata.get_supplier_settlements_summary(
                date_from=_normalize_relative_date(date_from),
                date_to=_normalize_relative_date(date_to),
            ...
            return _ok(
                {
                    "intent": "get_supplier_settlements_summary",
...
def get_supplier_settlements_summary(
    ...
    """Показать read-only сводку по взаиморасчетам с поставщиками.

    Возвращает безопасную управленческую оценку кредиторки на основе поступлений/счетов поставщика
    и исходящих оплат, без raw OData и без записи в 1С.
    """
    try:
        result = odata.get_supplier_settlements_summary(
            date_from=date_from,
